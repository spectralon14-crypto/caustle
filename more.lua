...
local THEME = {
	...
	PlatformAccent = Color3.fromRGB(255, 196, 64),
...
local currentCPM = tonumber(Config.TypingCPM or Config.CPM) or 550
local isBlatant = Config.Blatant
local useHumanization = Config.HumanTyping ~= false
...
LoadingGui.Name = "UltimateWordHelperLoading"
...
LoadingTitle.Text = "Ultimate Word Helper"
...
if env.UltimateWordHelperInstance and env.UltimateWordHelperInstance.Parent then
	env.UltimateWordHelperInstance:Destroy()
end
...
env.UltimateWordHelperInstance = ScreenGui
...
Title.Text = "Ultimate <font color=\"rgb(114,100,255)\">Word Helper</font>"
Title.RichText = true
...
local function SetupSlider(btn, bg, fill, callback)
	btn.MouseButton1Down:Connect(function()
		...
		local function Update()
			...
			Config.TypingCPM = currentCPM
			Config.DeleteCPM = tonumber(Config.DeleteCPM) or 420
			Config.HumanTyping = Config.HumanTyping ~= false
			Config.HumanDelete = Config.HumanDelete ~= false
			Config.TypingProfile = Config.TypingProfile or "Normal"
			Config.SafeMode = Config.SafeMode == true
...
LayoutDropdown.Position = UDim2.new(0, 175, 0, 145)
...
SliderLabel.Text = "Typing CPM: " .. currentCPM
...
local HumanizeBtn = CreateToggle("Human Type: "..((Config.HumanTyping ~= false) and "ON" or "OFF"), UDim2.new(0, 15, 0, 5), function()
	Config.HumanTyping = not (Config.HumanTyping ~= false)
	Config.Humanize = Config.HumanTyping
	Config.TypingProfile = "Custom"
	useHumanization = Config.HumanTyping
	return Config.HumanTyping, "Human Type: "..(Config.HumanTyping and "ON" or "OFF"), Config.HumanTyping and THEME.Success or Color3.fromRGB(255, 100, 100)
end)
HumanizeBtn.TextColor3 = (Config.HumanTyping ~= false) and THEME.Success or Color3.fromRGB(255, 100, 100)

local FingerBtn = CreateToggle("10-Finger: "..(useFingerModel and "ON" or "OFF"), UDim2.new(0, 122, 0, 5), function()
...
local KeyboardBtn = CreateToggle("Keyboard: "..(showKeyboard and "ON" or "OFF"), UDim2.new(0, 229, 0, 5), function()
...
SortBtn.Size = UDim2.new(0, 150, 0, 24)

local AutoBtn = CreateToggle("Auto Play: "..(autoPlay and "ON" or "OFF"), UDim2.new(0, 175, 0, 33), function()
...
AutoBtn.Size = UDim2.new(0, 150, 0, 24)
...
AutoJoinBtn.Size = UDim2.new(0, 310, 0, 24)
...
CreateCheckbox("4 Player", UDim2.new(0, 122, 0, 88), "_4p")
CreateCheckbox("8 Player", UDim2.new(0, 229, 0, 88), "_8p")
...
BlatantBtn.Size = UDim2.new(0, 150, 0, 24)

local RiskyBtn = CreateToggle("Risky Mistakes: "..(riskyMistakes and "ON" or "OFF"), UDim2.new(0, 175, 0, 115), function()
...
RiskyBtn.Size = UDim2.new(0, 150, 0, 24)
...
ManageWordsBtn.Size = UDim2.new(0, 150, 0, 24)
...
WordBrowserBtn.Size = UDim2.new(0, 310, 0, 24)
...
ServerBrowserBtn.Size = UDim2.new(0, 310, 0, 24)
...
local function CalculateDelayForKeys(prevChar, nextChar)
	local effectiveCPM = math.max(1, currentCPM)
	if Config.HumanTyping ~= false then
		effectiveCPM = math.clamp(effectiveCPM, 120, 900)
	elseif isBlatant then
		return 60 / effectiveCPM
	end
	...
	local distanceExtra = 0
	if useFingerModel and prevChar and nextChar and prevChar ~= "" then
		local dist = KeyDistance(prevChar, nextChar)
		distanceExtra = dist * 0.012 * (520 / math.max(180, effectiveCPM))
		local pa = KEY_POS[prevChar:lower()]
		local pb = KEY_POS[nextChar:lower()]
		if pa and pb then
			local sameSide = (pa.x <= 5 and pb.x <= 5) or (pa.x > 5 and pb.x > 5)
			if sameSide then distanceExtra = distanceExtra * 0.82 end
		end
	end

	local centered = ((math.random() + math.random() + math.random() + math.random()) / 4) - 0.5
	local delay = baseDelay * (1 + centered * 0.42) + distanceExtra
	if prevChar and nextChar and prevChar:lower() == nextChar:lower() then
		delay = delay * 0.92
	end
	if math.random() < 0.045 then
		delay = delay + 0.035 + math.random() * 0.11
	end
	return math.clamp(delay, baseDelay * 0.58, math.max(baseDelay * 2.5, 0.28))
end
...
local function Backspace(count)
	...
	local focused = UserInputService:GetFocusedTextBox() or GetGameTextBox()
	if Config.HumanDelete == false then
		if focused and focused:IsDescendantOf(game) and focused.TextEditable then
			local textValue = focused.Text or ""
			focused.Text = textValue:sub(1, math.max(0, #textValue - count))
		else
			for _ = 1, count do
				pcall(function()
	...
	local deleteCPM = math.clamp(tonumber(Config.DeleteCPM) or 420, 120, 900)
	local baseDeleteDelay = 60 / deleteCPM
	for i = 1, count do
		if unloaded then break end
		local activeBox = UserInputService:GetFocusedTextBox()
		if activeBox and activeBox:IsDescendantOf(game) and activeBox.TextEditable then
			focused = activeBox
		end

		local before = focused and focused.Text or nil
		local hold = math.clamp(0.012 + math.random() * 0.022, 0.012, 0.04)
		...
		if focused and focused:IsDescendantOf(game) and focused.TextEditable then
			...
			if (not sent) or focused.Text == before then
				local currentText = focused.Text or ""
				focused.Text = currentText:sub(1, math.max(0, #currentText - 1))
			end
		end

		local centered = ((math.random() + math.random() + math.random() + math.random()) / 4) - 0.5
		local delay = baseDeleteDelay * (1 + centered * 0.38)
		if count >= 8 then
			local progress = math.clamp(i / count, 0, 1)
			delay = delay * (0.94 + progress * 0.12)
		end
		if math.random() < 0.055 then
			delay = delay + 0.025 + math.random() * 0.09
		end
		task.wait(math.clamp(delay, baseDeleteDelay * 0.6, math.max(baseDeleteDelay * 2.35, 0.26)))
	end
	lastKey = nil
end
...
local function SmartType(targetWord, currentDetected, isCorrection, bypassTurn)
	...
	if Config.SafeMode == true then
		StatusText.Text = "Safe Mode: preview only"
		StatusText.TextColor3 = THEME.Warning
		ShowToast("Safe Mode blocked in-game typing.", "warning")
		return
	end
	...
	local success, err = pcall(function()
		if isCorrection then
			...
			for i = 1, #toType do
				...
				lastKey = ch
				if Config.HumanTyping ~= false and math.random() < 0.025 then
					task.wait(0.04 + math.random() * 0.13)
				end
			end
			...
			if not accepted then
				...
				local focused = GetGameTextBox() or UserInputService:GetFocusedTextBox()
				local deleteCount = (focused and #(focused.Text or "")) or (#targetWord + 5)
...
SetupSlider(SliderBtn, SliderBg, SliderFill, function(pct)
	currentCPM = math.floor(120 + pct * (900 - 120) + 0.5)
	Config.CPM = currentCPM
	Config.TypingCPM = currentCPM
	Config.TypingProfile = "Custom"
...
-- Build the extra controls in a separate function scope so the original script's local-variable budget stays intact.
task.defer(function()
	local typingCPM = currentCPM
	local deleteCPM = math.clamp(tonumber(Config.DeleteCPM) or 420, 120, 900)
	local panelBusy = false
	local profileOrder = {"Calm", "Normal", "Fast", "Custom"}
	local profileValues = {
		Calm = {typing = 300, deleting = 270, think = 0.95},
		Normal = {typing = 480, deleting = 400, think = 0.72},
		Fast = {typing = 700, deleting = 580, think = 0.48},
	}

	local openButton = Instance.new("TextButton", TogglesFrame)
	openButton.Text = "Human Lab"
	openButton.Font = Enum.Font.GothamMedium
	openButton.TextSize = 11
	openButton.TextColor3 = THEME.PlatformAccent
	openButton.BackgroundColor3 = THEME.Background
	openButton.Size = UDim2.new(0, 310, 0, 24)
	openButton.Position = UDim2.new(0, 15, 0, 235)
	Instance.new("UICorner", openButton).CornerRadius = UDim.new(0, 4)

	local panel = Instance.new("Frame", ScreenGui)
	panel.Name = "TypingControlsPreview"
	panel.Size = UDim2.new(0, 390, 0, 455)
	panel.Position = UDim2.new(0.5, -195, 0.5, -228)
	...
	closeButton.Position = UDim2.new(1, -42, 0, 1)
	...
	local function makeButton(textValue, x, y, width)
	...
	local profileButton = makeButton("Profile", 15, 50, 170)
	local safeButton = makeButton("Safe Mode", 200, 50, 175)
	local humanTypeButton = makeButton("Human Type", 15, 84, 170)
	local humanDeleteButton = makeButton("Human Delete", 200, 84, 175)
	...
	local function makeSlider(y)
		...
		bg.Size = UDim2.new(1, -30, 0, 7)
		bg.Position = UDim2.new(0, 15, 0, y + 23)
		bg.BackgroundColor3 = THEME.Slider
		Instance.new("UICorner", bg).CornerRadius = UDim.new(1, 0)
		local fill = Instance.new("Frame", bg)
		fill.Size = UDim2.new(0.5, 0, 1, 0)
		fill.BackgroundColor3 = THEME.PlatformAccent
		Instance.new("UICorner", fill).CornerRadius = UDim.new(1, 0)
		local hitbox = Instance.new("TextButton", bg)
		hitbox.Size = UDim2.new(1, 0, 1, 0)
		hitbox.BackgroundTransparency = 1
		hitbox.Text = ""
		return label, bg, fill, hitbox
	end

	local typingLabel, typingBg, typingFill, typingHitbox = makeSlider(124)
	local deleteLabel, deleteBg, deleteFill, deleteHitbox = makeSlider(174)

	local sourceBox = Instance.new("TextBox", panel)
	sourceBox.Size = UDim2.new(1, -30, 0, 30)
	sourceBox.Position = UDim2.new(0, 15, 0, 226)
	sourceBox.BackgroundColor3 = THEME.ItemBG
	sourceBox.TextColor3 = THEME.Text
	sourceBox.PlaceholderColor3 = THEME.SubText
	sourceBox.PlaceholderText = "Word to preview..."
	sourceBox.Text = "pneumonoultramicroscopicsilicovolcanoconiosis"
	sourceBox.ClearTextOnFocus = false
	sourceBox.Font = Enum.Font.Gotham
	sourceBox.TextSize = 12
	Instance.new("UICorner", sourceBox).CornerRadius = UDim.new(0, 5)

	local outputBox = Instance.new("TextBox", panel)
	outputBox.Size = UDim2.new(1, -30, 0, 58)
	outputBox.Position = UDim2.new(0, 15, 0, 264)
	outputBox.BackgroundColor3 = Color3.fromRGB(27, 27, 33)
	outputBox.TextColor3 = THEME.Success
	outputBox.PlaceholderColor3 = THEME.SubText
	outputBox.PlaceholderText = "Typing preview appears here"
	outputBox.Text = ""
	...
	local testTypeButton = makeButton("Test Type", 15, 332, 112)
	local testDeleteButton = makeButton("Test Delete", 139, 332, 112)
	local clearButton = makeButton("Clear", 263, 332, 112)
	local status = Instance.new("TextLabel", panel)
	status.Size = UDim2.new(1, -30, 0, 48)
	status.Position = UDim2.new(0, 15, 0, 372)
	...
	local function refresh()
		typingCPM = currentCPM
		deleteCPM = math.clamp(tonumber(Config.DeleteCPM) or deleteCPM, 120, 900)
		profileButton.Text = "Profile: " .. (Config.TypingProfile or "Normal")
		safeButton.Text = "Safe Mode: " .. ((Config.SafeMode == true) and "ON" or "OFF")
		safeButton.TextColor3 = (Config.SafeMode == true) and THEME.Warning or THEME.SubText
		humanTypeButton.Text = "Human Type: " .. ((Config.HumanTyping ~= false) and "ON" or "OFF")
		humanTypeButton.TextColor3 = (Config.HumanTyping ~= false) and THEME.Success or Color3.fromRGB(255, 100, 100)
		humanDeleteButton.Text = "Human Delete: " .. ((Config.HumanDelete ~= false) and "ON" or "OFF")
		humanDeleteButton.TextColor3 = (Config.HumanDelete ~= false) and THEME.Success or Color3.fromRGB(255, 100, 100)
		typingLabel.Text = "Typing CPM: " .. typingCPM
		deleteLabel.Text = "Delete CPM: " .. deleteCPM
		typingFill.Size = UDim2.new((typingCPM - 120) / 780, 0, 1, 0)
		deleteFill.Size = UDim2.new((deleteCPM - 120) / 780, 0, 1, 0)
		HumanizeBtn.Text = "Human Type: " .. ((Config.HumanTyping ~= false) and "ON" or "OFF")
		HumanizeBtn.TextColor3 = (Config.HumanTyping ~= false) and THEME.Success or Color3.fromRGB(255, 100, 100)
	end

	local function setTyping(value, custom)
		currentCPM = math.clamp(math.floor(value + 0.5), 120, 900)
		Config.CPM = currentCPM
		Config.TypingCPM = currentCPM
		if custom then Config.TypingProfile = "Custom" end
		SliderFill.Size = UDim2.new((currentCPM - 120) / 780, 0, 1, 0)
		SliderLabel.Text = "Typing CPM: " .. currentCPM
		refresh()
	end

	local function setDeleting(value, custom)
		Config.DeleteCPM = math.clamp(math.floor(value + 0.5), 120, 900)
		if custom then Config.TypingProfile = "Custom" end
		refresh()
	end

	local function connectSlider(hitbox, bg, callback)
		hitbox.MouseButton1Down:Connect(function()
			local moveConnection, releaseConnection
			local function update()
				local mouse = UserInputService:GetMouseLocation()
				local relative = math.clamp(mouse.X - bg.AbsolutePosition.X, 0, bg.AbsoluteSize.X)
				callback(relative / bg.AbsoluteSize.X)
			end
			update()
			moveConnection = RunService.RenderStepped:Connect(update)
	...
	connectSlider(typingHitbox, typingBg, function(percent) setTyping(120 + percent * 780, true) end)
	connectSlider(deleteHitbox, deleteBg, function(percent) setDeleting(120 + percent * 780, true) end)

	profileButton.MouseButton1Click:Connect(function()
		local profile = Config.TypingProfile or "Normal"
		local index = table.find(profileOrder, profile) or 2
		profile = profileOrder[(index % #profileOrder) + 1]
		Config.TypingProfile = profile
		if profileValues[profile] then
			Config.HumanTyping = true
			Config.HumanDelete = true
			Config.Humanize = true
			useHumanization = true
			setTyping(profileValues[profile].typing, false)
			setDeleting(profileValues[profile].deleting, false)
			thinkDelayCurrent = profileValues[profile].think
			Config.ThinkDelay = thinkDelayCurrent
		end
		refresh()
		SaveConfig()
	...
	safeButton.MouseButton1Click:Connect(function()
		Config.SafeMode = not (Config.SafeMode == true)
		refresh()
		SaveConfig()
	end)
	humanTypeButton.MouseButton1Click:Connect(function()
		Config.HumanTyping = not (Config.HumanTyping ~= false)
		Config.Humanize = Config.HumanTyping
		Config.TypingProfile = "Custom"
		useHumanization = Config.HumanTyping
	...
	humanDeleteButton.MouseButton1Click:Connect(function()
		Config.HumanDelete = not (Config.HumanDelete ~= false)
		Config.TypingProfile = "Custom"
		refresh()
		SaveConfig()
	end)
	...
	local function previewDeleteDelay(index, total)
		local cpm = math.clamp(tonumber(Config.DeleteCPM) or 420, 120, 900)
		local base = 60 / cpm
		if Config.HumanDelete == false then return 0 end
		local centered = ((math.random() + math.random() + math.random() + math.random()) / 4) - 0.5
		local delay = base * (1 + centered * 0.38)
		if total >= 8 then delay = delay * (0.94 + math.clamp(index / total, 0, 1) * 0.12) end
		if math.random() < 0.055 then delay = delay + 0.025 + math.random() * 0.09 end
		return math.clamp(delay, base * 0.6, math.max(base * 2.35, 0.26))
	end

	testTypeButton.MouseButton1Click:Connect(function()
		if panelBusy then return end
		panelBusy = true
		local word = sourceBox.Text:gsub("%s+", "")
		if word == "" then word = "wordhelper" end
		outputBox.Text = ""
		status.Text = "Typing preview..."
		local previous = nil
		for i = 1, #word do
			local character = word:sub(i, i)
			outputBox.Text = outputBox.Text .. character
			task.wait(CalculateDelayForKeys(previous, character))
			previous = character
		end
		status.Text = "Finished typing at target " .. currentCPM .. " CPM."
		panelBusy = false
	end)

	testDeleteButton.MouseButton1Click:Connect(function()
		if panelBusy then return end
		panelBusy = true
		status.Text = "Deleting preview one character at a time..."
		local total = #outputBox.Text
		if Config.HumanDelete == false then
			outputBox.Text = ""
		else
			for i = 1, total do
				outputBox.Text = outputBox.Text:sub(1, math.max(0, #outputBox.Text - 1))
				task.wait(previewDeleteDelay(i, total))
			end
		end
		status.Text = "Finished deleting at target " .. (tonumber(Config.DeleteCPM) or 420) .. " CPM."
		panelBusy = false
	end)
	...
	clearButton.MouseButton1Click:Connect(function()
		if panelBusy then return end
		outputBox.Text = ""
		status.Text = "Preview cleared."
	end)

	Config.CPM = currentCPM
	Config.TypingCPM = currentCPM
	Config.DeleteCPM = deleteCPM
	Config.HumanTyping = Config.HumanTyping ~= false
	Config.HumanDelete = Config.HumanDelete ~= false
	Config.TypingProfile = Config.TypingProfile or "Normal"
	Config.SafeMode = Config.SafeMode == true
	setTyping(currentCPM, false)
	setDeleting(deleteCPM, false)
	status.Text = "Preview uses the same pacing model as in-game typing and deletion."
	refresh()
end)
...
runConn = RunService.RenderStepped:Connect(function()
	local success, err = pcall(function()
		...
		if Config.SafeMode ~= true and isVisible and isMyTurn and not isTyping and seconds and seconds < 1.5 then
		...
		if autoPlay and Config.SafeMode ~= true and not isTyping and not isAutoPlayScheduled and currentBestMatch and detected == lastDetected then
			...
			if isMyTurnCheck then
				...
				task.spawn(function()
					...
					if autoPlay and Config.SafeMode ~= true and not isTyping and GetCurrentGameWord() == snapshotDetected and stillMyTurn then
